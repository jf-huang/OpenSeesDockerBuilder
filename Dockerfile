...
RUN tar -xzf metis-5.1.0.tar.gz && cd metis-5.1.0 && \
...
RUN tar -xzf scotch_6.0.9.tar.gz && cd scotch_6.0.9 && \
...
RUN tar -xzf MUMPS_5.2.1.tar.gz && cd MUMPS_5.2.1 && \
...
# hdf5
RUN tar -xzf CMake-hdf5-1.10.4.tar.gz && cd CMake-hdf5-1.10.4/ && cd hdf5-1.10.4 && \
...
# parallel version
RUN git clone https://github.com/jf-huang/OpenSees.git && cd OpenSees &&  \
