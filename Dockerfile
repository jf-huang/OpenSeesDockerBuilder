...
ENV DEBIAN_FRONTEND=noninteractive \
    mainDir=/home/jfhuang
WORKDIR $mainDir

# basic packages
RUN apt-get -y update && \
apt-get -y install build-essential gcc gfortran make dkms tcl8.6-dev zlib1g-dev flex bison python git
#apt-get -y upgrade

COPY mpich2-1.5rc3.tar.gz blas-3.8.0.tgz lapack-3.9.0.tar.gz scalapack-2.1.0.tar.gz \
     metis-5.1.0.tar.gz parmetis-4.0.3.tar.gz scotch_6.0.9.tar.gz mumps_5.1.2.orig.tar.gz \
     v2.2.1.petsc.tar.gz CMake-hdf5-1.10.4.tar.gz ./
...
# sequential version
#RUN cd OpenSees && rm Makefile.def && cp Makefile_SEQ.def Makefile.def && \
#sed -i 's:HOME		= /home/jfhuang:HOME		= ${mainDir}:g' Makefile.def && \
#make wipe && make
# parallel version
RUN cd OpenSees && rm Makefile.def && cp Makefile_PARALLEL.def Makefile.def && \
sed -i 's:HOME  = /home/jfhuang:HOME  = ${mainDir}:g' Makefile.def && \
...
RUN sed '$a export PATH=$PATH:$mainDir-mpiInstall/bin;' /etc/profile && \
sed '$a export PATH=$PATH:$mainDir/bin;' /etc/profile
